...
contract MXCToken is StandardToken {
    ...
    event VestedTokenRedeemed(
        address indexed _to,
        uint256 _amount,
        uint256 _vestedMonths
    );
    event GrantedTokenReturned(
        address indexed _from,
        address indexed _to,
        uint256 _amount
    );
        ...
        returns (uint256 amount, uint256 vestedMonths, uint256 curTime)
    {
        ...
        require(
            timeLocks[_to].amount > 0,
            "Nothing was granted to this address."
        );
        ...
        if (vestedMonths > 0) {
            vestTotalMonths =
                timeLocks[_to].vesting.sub(timeLocks[_to].start) /
                MONTH;
    ...
    function returnGrantedToken(uint256 _amount) public returns (bool success) {
        ...
        require(
            _amount <=
                timeLocks[msg.sender].amount.sub(
                    timeLocks[msg.sender].vestedAmount
                ),
            "Not enough granted token to return."
        );

        timeLocks[msg.sender].amount = timeLocks[msg.sender].amount.sub(
            _amount
        );
